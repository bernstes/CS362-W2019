...
int main(){
    ...
    for(i = 0; i < numTests; i++) {
        ...
        if(rand_assert_true(compareInt(state.handCount[0] + 2, testState.handCount[0]))){
            printf("\nTest 1: Play Smithy and Draw 3 Cards (net +2 after discard)\n");
            rand_expectation(state.handCount[0] + 2, testState.handCount[0]);
            test1++;
        }

        //Make sure the 3 cards are drawn from the player's own deck
        if(rand_assert_true(compareInt(state.deckCount[0] - 3, testState.deckCount[0]))){
            printf("\nTest 2: Test for proper change in current player's deck count\n");
            rand_expectation(state.deckCount[0] - 3, testState.deckCount[0]);
            test2++;
        }
        ...
        //printf("\nTest 7: Test for no changes in kingdom card piles\n");
        if(rand_assert_true(compareInt(state.supplyCount[smithy], testState.supplyCount[smithy]))){
            printf("Test 7: Kingdom Card 1 Mismatch\n");
            rand_expectation(state.supplyCount[smithy], testState.supplyCount[smithy]);
            test7++;
        }
